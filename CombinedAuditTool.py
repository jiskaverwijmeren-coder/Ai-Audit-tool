...
if thema == "donker":
    ...
    css = """<style>
        .stApp { background-color: #fff9f0; color: #1a1a1a; }
        section[data-testid="stSidebar"] { background-color: #fff0f5; border-right: 2px solid #f9a8d4; }
        .stButton > button { background-color: #f472b6; color: white; border: none; border-radius: 8px; font-weight: 600; }
        .stButton > button:hover { background-color: #ec4899; color: white; }
        [data-testid="stMetric"] { background-color: #fef9c3; border: 1px solid #fde68a; border-radius: 10px; padding: 12px; }
        [data-testid="stExpander"] { border: 1px solid #f9a8d4; border-radius: 8px; background-color: #fff5f9; }
        h1, h2, h3 { color: #be185d; }
        hr { border-color: #fda4af; }
        [data-testid="stChatInput"] { background-color: #fff0f5 !important; border: 1px solid #f9a8d4 !important; border-radius: 8px !important; }
        [data-testid="stChatInput"] textarea { background-color: transparent !important; color: #1a1a1a !important; }
        [data-testid="stBottom"] > div { background-color: #fff9f0 !important; }
        .doc-card { background-color: #fff0f5; border: 1px solid #f9a8d4; color: #1a1a1a; }
        .transcription-box { background-color: #fff0f5; border: 1px solid #f9a8d4; color: #1a1a1a; }
    </style>"""
...
st.caption("Upload documenten · Detecteer risico's · Genereer auditbevindingen & ISO-analyses per document")
...
# =========================
# SECTION 1 — LIVE MIC RECORDING
# =========================

st.subheader("🎤 Live opnemen via microfoon")
st.caption("Neem direct op via je microfoon. De transcriptie kun je daarna toevoegen aan de documentenlijst.")
...
if audio_data is not None and len(audio_data) > 0:
    ...
    with st.spinner("Transcriberen met Whisper..."):
...
if "transcribed_text" in st.session_state and st.session_state.transcribed_text:
    if st.button("➕ Voeg microfoonopname toe aan documentenlijst"):
        name = "🎤 Microfoonopname"
        ...
        if name not in existing_names:
            ...
            st.info("🔄 Bestaande microfoonopname bijgewerkt.")
        st.rerun()
...
if audio_file is not None:
    # Show audio player so user can listen back
...
if not st.session_state.document_list:
    st.info("Nog geen documenten toegevoegd. Upload bestanden, een audiobestand of gebruik de microfoon hierboven.")
else:
